...
from sqlalchemy import Enum, Integer, ForeignKey, DateTime, UniqueConstraint, Index
...
class ProfileAssignment(Base):
    ...
    source: Mapped[str] = mapped_column(Enum(AssignmentSource, native_enum=False, length=20))
    source_id: Mapped[int | None] = mapped_column(Integer, nullable=True)
    status: Mapped[str] = mapped_column(Enum(AssignmentStatus, native_enum=False, length=20), default=AssignmentStatus.PENDING)
