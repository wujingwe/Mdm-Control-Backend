...
class ProfileRepository:
    ...
    async def update_assignment_reports(
        ...
        if not reports:
            return
        stmt = select(ProfileAssignment.id).where(
            ProfileAssignment.device_id == device_id,
            ProfileAssignment.id.in_(reports),
        )
        result = await self._db.execute(stmt)
        ...
        for assignment_id in valid_ids:
            ...
            await self._db.execute(
                update(ProfileAssignment).where(ProfileAssignment.id == assignment_id).values(**values)
            )
        await self._db.commit()
    ...
    async def _load_assignments(self, assignment_ids: Mapping[int, object]) -> list[ProfileAssignment]:
        result = await self._db.execute(select(ProfileAssignment).where(ProfileAssignment.id.in_(assignment_ids)))
        return list(result.scalars().all())
