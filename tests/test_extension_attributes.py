...
class TestExtensionAttributeRepository:
    async def test_crud(self, db_session):
        ...
        updated = await repo.update(created.id, ExtensionAttributeUpdate(description="Updated desc"))
    ...
    async def test_unique_name(self, db_session):
        ...
        await repo.create(ExtensionAttributeCreate(
            name="Dept", data_type=ExtensionDataType.STRING, input_type=ExtensionInputType.TEXT_FIELD, created_by=1,
        ))
        with pytest.raises(ConflictError):
            await repo.create(ExtensionAttributeCreate(
                name="Dept", data_type=ExtensionDataType.INTEGER, input_type=ExtensionInputType.TEXT_FIELD, created_by=1,
            ))

    async def test_list(self, db_session):
        repo = ExtensionAttributeRepository(db_session)
        await repo.create(ExtensionAttributeCreate(
            name="A", data_type=ExtensionDataType.STRING, input_type=ExtensionInputType.TEXT_FIELD, created_by=1,
        ))
        await repo.create(ExtensionAttributeCreate(
            name="B", data_type=ExtensionDataType.INTEGER, input_type=ExtensionInputType.TEXT_FIELD, created_by=1,
        ))
    ...
    async def test_delete(self, db_session):
        repo = ExtensionAttributeRepository(db_session)
        created = await repo.create(ExtensionAttributeCreate(
            name="X", data_type=ExtensionDataType.STRING, input_type=ExtensionInputType.TEXT_FIELD, created_by=1,
        ))
