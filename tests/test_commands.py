...
class TestCommandService:
    ...
    async def test_trigger_command_success(self, repo):
    ...
    async def test_trigger_command_publish_fails(self, repo):
        svc = CommandService(repo)
        data = CommandCreate(command_type=CommandType.LOCK)
        with patch("app.commands.services.rabbitmq_producer") as mock_producer:
    ...
    async def test_get_command(self, repo):
        fake = MagicMock()
    ...
    async def test_list_commands(self, repo):
        svc = CommandService(repo)
    ...
    async def test_list_device_commands(self, repo):
        svc = CommandService(repo)
    ...
    async def test_cancel_command(self, repo):
        fake = MagicMock()
    ...
    async def test_cancel_command_not_cancellable(self, repo):
