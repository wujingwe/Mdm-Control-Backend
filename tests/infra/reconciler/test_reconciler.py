...
# ── recalculate_profile with smart group scope ───────────────────────
...
# ── recalculate_profile with static group scope ──────────────────────
...
# ── recalculate_mobile_app with smart group scope ─────────────────────
...
# ── recalculate_mobile_app with static group scope ────────────────────
...
class TestResolveScope:
    ...
    async def test_exclusion_with_no_device_ids(self, db_session: AsyncSession) -> None:
        ...
        assert (ScopeType.ALL_DEVICES, None) in resolved
        assert device.id in resolved[(ScopeType.ALL_DEVICES, None)]
...
class TestResolveTarget:
    async def test_static_group_empty(self, db_session: AsyncSession) -> None:
        ...
        result = await reconciler._resolve_target_ids(target)
        assert result == set()
...
class TestResolveExclusion:
    async def test_device_exclusion(self, db_session: AsyncSession) -> None:
        ...
        result = await reconciler._resolve_exclusion_ids(exclusion)
        assert result == {42}
...
class TestSendPushMessages:
    async def test_push_skipped_without_assignment(self, db_session: AsyncSession) -> None:
        """A device with no assignment row must not receive an untracked push."""
        producer = _make_producer()
        profile_repo = ProfileRepository(db_session)
        reconciler = AssignmentReconciler(
            profile_repo,
...
class TestSendRevokeMessages:
    async def test_revoke_skipped_without_assignment(self, db_session: AsyncSession) -> None:
        """A device with no assignment row must not receive an untracked revoke."""
        producer = _make_producer()
        profile_repo = ProfileRepository(db_session)
        reconciler = AssignmentReconciler(
            profile_repo,
...
class TestSendMobileAppPushMessages:
    ...
    async def test_exception_with_assignment(self, db_session: AsyncSession) -> None:
        ...
        failed_assignment = await app_repo._db.get(MobileAppAssignment, assignments[0].id)
...
class TestRecalculateProfileGroupTriggers:
    async def test_smart_group_triggers_no_scope_match(self, db_session: AsyncSession) -> None:
        ...
        assert await profile_repo.get_current_assignments(profile.id) is not None
...
class TestSendPushMessagesExceptionWithAssignment:
    async def test_exception_with_assignment_marks_failed(self, db_session: AsyncSession) -> None:
        ...
        failed_assignment = await profile_repo._db.get(ProfileAssignment, assignments[0].id)
        assert failed_assignment is not None
        assert failed_assignment.status == AssignmentStatus.FAILED
