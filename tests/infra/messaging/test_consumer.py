...
class TestProcessDeviceCommandStatus:
    ...
    async def test_completed_updates_command(self, mock_session_factory: Any) -> None:
        from app.infra.messaging.consumer import process_device_command_status
...
class TestCompositeHandler:
    @pytest.mark.asyncio
    @patch("app.infra.messaging.consumer.process_profile_status_message", new_callable=AsyncMock)
    async def test_routes_profile_event(self, mock_profile: Any) -> None:
        from app.infra.messaging.consumer import composite_handler
    ...
    @pytest.mark.asyncio
    @patch("app.infra.messaging.consumer.process_device_command_status", new_callable=AsyncMock)
